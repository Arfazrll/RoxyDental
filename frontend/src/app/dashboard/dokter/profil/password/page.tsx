  ...
  const handleConfirmChange = async () => {
    ...
    try {
      ...
      if (error?.message === "Password saat ini salah") {
        ...
        showToast("error", error?.message || "Terjadi kesalahan saat mengganti password");
      }
  ...
  return (
    <div className="min-h-screen bg-linear-to-b from-[#FFF5F7] to-white">
      ...
      <div className="pt-6 px-4 lg:px-8">
                      ...
                      <div key={type} className={`${idx === 0 ? 'mt-4' : ''}`}>
                        ...
                        <div className="relative">
                          <Input
                            ...
                            className={`border-pink-200 pr-12 focus:border-pink-500 focus:ring-2 focus:ring-pink-100 rounded-lg transition-shadow duration-150 shadow-sm ${
                              showError ? 'border-red-400 ring-red-50' : 'hover:shadow-md'
                            }`}
                            disabled={loading}
                          />
        ...
        <p className="text-center text-sm text-pink-600 mt-8"> © 2025 POLADC — All rights reserved</p>
      </div>
      ...
      {toast.type && (
        <div className="fixed right-6 top-6 z-50">
          <div
            className={`max-w-xs w-full rounded-lg shadow-lg p-3 flex items-start gap-3 transform transition-transform duration-200 ${
